function Map(){
  ...
  this.floor_tiles = main.resources.zone.floor_tiles;
  this.width = main.resources.zone.floor_tiles.length;
  this.height = main.resources.zone.floor_tiles[0].length;
  ...
  this.Draw = function(ctx){
    ...
    ctx.save();
      ...
      ctx.translate(center_x_ctx,center_y_ctx);
      for(i = Math.floor(init_h); i <= Math.ceil(end_h); i++){
        for(j = Math.floor(init_v); j <= Math.ceil(end_v);j++){
          var x = i*this.grid - this.center_x;
          var y = j*this.grid - this.center_y;
